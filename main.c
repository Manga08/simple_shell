 ...
 * Return: 0
 */
int exit_func(char *arg0, char **buffercpy, char **buffer, int procces)
{
	char *exit_arg[2];
	int error_e;
...
{
	char *tmp = NULL, *buffer = NULL, *buffercpy = NULL;
	size_t bufsize = 1024;
	int atty, status = 0, procces = 0;
	...
	while (1)
	{
		...
		status = exit_func(arg[0], &buffercpy, &buffer, procces);
		if (status != -1)
